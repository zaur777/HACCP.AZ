...
async function startServer() {
  ...
  try {
    ...
    // 2. Check for existing user before opening a connection
    ...
    try {
      ...
      const companyRes = await client.query(
        ...
        [companyName, regNumber || null, address || null, industryType || null, responsiblePerson || null]
      );
      ...
      // 5. Create initial HACCP plan (using column names from your schema)
      await client.query(
        `INSERT INTO haccp_plans (company_id, product_description, flow_diagram, hazard_analysis, critical_limits) 
         VALUES ($1, $2, $3, $4, $5)`,
      ...
      console.error("DB DETAIL:", dbErr.detail); // This reveals specific constraint names
      throw dbErr; // Pass it to the outer catch
    } finally {
      client.release();
    }

  } catch (err: any) {
    console.error("FINAL REGISTRATION CATCH:", err.message);
    // This ensures that NO MATTER WHAT, the frontend gets valid JSON
    if (!res.headersSent) {
      res.status(500).json({ 
