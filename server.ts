...
async function seedInitialData() {
  try {
    ...
    const { rows: templateRows } = await pool.query("SELECT id FROM haccp_templates LIMIT 1");
    if (templateRows.length === 0) {
      console.log("No HACCP templates found. Seeding initial templates...");
      const templates = [
        {
          name: "Bakery & Pastry",
          image: "https://images.unsplash.com/photo-1509440159596-0249088772ff?auto=format&fit=crop&q=80&w=800",
          product_description: "Production of fresh bread, rolls, and sweet pastries. Ingredients include flour, water, yeast, sugar, eggs, and dairy.",
          flow_diagram: "1. Receiving -> 2. Storage -> 3. Mixing -> 4. Proofing -> 5. Baking -> 6. Cooling -> 7. Packaging -> 8. Distribution",
          hazard_analysis: "Biological: Mold growth in storage, Salmonella in eggs. Physical: Metal fragments from mixers. Chemical: Cleaning agent residues.",
          ccp_determination: "CCP 1: Baking (Temperature control). CCP 2: Storage (Humidity control).",
          critical_limits: "Baking: Internal temperature > 85°C. Storage: < 60% humidity.",
          monitoring_procedures: "Continuous oven temperature logging. Daily humidity checks in dry storage.",
          corrective_actions_plan: "Extend baking time if temp not reached. Adjust HVAC if humidity exceeds limit."
        },
        {
          name: "Meat Processing",
          image: "https://images.unsplash.com/photo-1607623814075-e51df1bdc82f?auto=format&fit=crop&q=80&w=800",
          product_description: "Processing of raw meat into cuts and minced products. High risk of cross-contamination and bacterial growth.",
          flow_diagram: "1. Intake -> 2. Cold Storage -> 3. Butchery -> 4. Grinding -> 5. Vacuum Packing -> 6. Dispatch",
          hazard_analysis: "Biological: E. coli, Listeria. Physical: Bone fragments. Chemical: Sanitizer carryover.",
          ccp_determination: "CCP 1: Cold Storage (Temp). CCP 2: Metal Detection.",
          critical_limits: "Cold Storage: < 4°C. Metal Detection: No fragments > 2mm.",
          monitoring_procedures: "Hourly fridge temp checks. Metal detector testing every shift.",
          corrective_actions_plan: "Move product to backup fridge if temp > 5°C. Re-scan batch if detector fails."
        }
      ];

      for (const t of templates) {
        await pool.query(
          `INSERT INTO haccp_templates 
           (name, image, product_description, flow_diagram, hazard_analysis, ccp_determination, critical_limits, monitoring_procedures, corrective_actions_plan) 
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
          [t.name, t.image, t.product_description, t.flow_diagram, t.hazard_analysis, t.ccp_determination, t.critical_limits, t.monitoring_procedures, t.corrective_actions_plan]
        );
      }
      console.log("HACCP templates seeded.");
    }
  } catch (err) {
...
async function startServer() {
  ...
  // HACCP Templates
  app.get("/api/haccp-templates", authenticate, async (req, res) => {
    try {
      const { rows } = await pool.query("SELECT * FROM haccp_templates ORDER BY created_at DESC");
      res.json(rows);
    } catch (err) {
      res.status(500).json({ error: "Failed to fetch HACCP templates" });
    }
  });

  app.post("/api/haccp-templates", authenticate, async (req: any, res) => {
    if (req.user.role !== 'SUPER_ADMIN') return res.status(403).json({ error: "Forbidden" });
    const { name, image, product_description, flow_diagram, hazard_analysis, ccp_determination, critical_limits, monitoring_procedures, corrective_actions_plan } = req.body;
    try {
      const { rows } = await pool.query(
        `INSERT INTO haccp_templates 
         (name, image, product_description, flow_diagram, hazard_analysis, ccp_determination, critical_limits, monitoring_procedures, corrective_actions_plan) 
  ...
  app.delete("/api/haccp-templates/:id", authenticate, async (req: any, res) => {
    if (req.user.role !== 'SUPER_ADMIN') return res.status(403).json({ error: "Forbidden" });
    try {
      await pool.query("DELETE FROM haccp_templates WHERE id = $1", [req.params.id]);
      res.json({ success: true });
    } catch (err) {
